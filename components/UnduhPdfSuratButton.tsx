...
// Tombol unduh PDF surat gembala (ber-watermark). Library PDF baru
// dimuat saat tombol diklik supaya halaman tetap ringan.
export default function UnduhPdfSuratButton({
  surat,
}: {
  surat: SuratGembala;
}) {
  ...
  const unduh = async () => {
    setStatus("proses");
    try {
      const { unduhPdfSurat } = await import("@/lib/pdf-surat");
      await unduhPdfSurat(surat);
      setStatus("siap");
    } catch (e) {
